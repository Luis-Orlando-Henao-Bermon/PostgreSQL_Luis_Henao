...
INSERT INTO ejemplo VALUES(
    ...
    '13:10:50', 
    '2025-02-23 13:10:50', 
    '2025-02-23 13:10:50+05', 
    '1 day', 
    '192.168.1.1', 
    '80:00:12:23:10:00', 
    '(10, 20)', 
    '{"key": "value"}', 
    'f66c844a-fd5b-48c3-8c46-735e41b0a2e6', 
    '15.23', 
    '[10, 20]', 
...
INSERT INTO usuarios(nombre,apellido,correo,ip,ciudad,blog,usuario,siguiendo,seguidores) VALUES
...
select usuario,'password','Usuario' from usuarios;
...
-- retorna una texto con variable
create function generate_ramdon_password2(long INTEGER)
returns TEXT as $$
declare
	texto TEXT;
BEGIN
	 texto=(SELECT string_agg(SUBSTRING('0123456789abcdefghijklmnopqrstuvwxyz!.', round(random() * length('0123456789abcdefghijklmnopqrstuvwxyz!.'))::integer, 1), '') as NewPassword
	FROM generate_series(0, long));
	return texto;
END;
$$ LANGUAGE plpgsql;
...
update credenciales set password=generate_ramdon_password2(12);
select * from credenciales;


select encode(sha512(password::bytea),'hex'), usuario from credenciales where usuario = 'brush99'; 
select encode(sha512('742bun0bzz'::bytea),'hex');
...
select * from credenciales where password_sha=sha512('742bun0bzz'::bytea) and usuario='brush99';s

--create extension if not exists pgcrypto;

--select encode(pgp_sym_encrypt('hola2', 'mi_clave_secreta'),'hex') as mensaje;

--select pgp_sym_encrypt(pgp_sym_encrypt('hola', 'mi_clave_secreta'),'mi_clave_secreta');
...
select random()::double precision||' Holaaa';

select generate_series(1,4);


update  credenciales set password=substring(md5(random()::text), 1, 10);

select * from usuarios order by 6 asc;	